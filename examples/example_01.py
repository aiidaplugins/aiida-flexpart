...
def test_run(flexpart_code):
    ...
    command = orm.Dict(
        dict={
            ...
            2,  #  Convection: 2 tiedtke, 1 emanuel, 0 no.
            'age_spectra':
    ...
    outgrid = orm.Dict(
        dict=read_yaml_data('outgrid.yaml', names=['Europe'])['Europe'])

    outgrid_nest = orm.Dict(
        dict=read_yaml_data('outgrid_nest.yaml', names=['Europe'])['Europe'])
    ...
    builder.model_settings = {
        ...
        release_settings,
        'locations':
        orm.Dict(dict=read_yaml_data('locations.yaml',
