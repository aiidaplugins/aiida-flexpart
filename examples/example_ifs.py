...
def test_run(flexpart_code):
    ...
    command = orm.Dict(dict=read_yaml_data('inputs/command.yaml'))

    outgrid = orm.Dict(
        dict=read_yaml_data('inputs/outgrid.yaml', names=['Europe'])['Europe'])

    outgrid_nest = orm.Dict(dict=read_yaml_data('inputs/outgrid_nest.yaml',
                                                names=['Europe'])['Europe'])

    release_settings = orm.Dict(dict=read_yaml_data('inputs/release.yaml'))

    locations = orm.Dict(dict=read_yaml_data('inputs/locations.yaml',
                                             names=['TEST_32', 'TEST_200']))

    # Links to the remote files/folders.
    glc = orm.RemoteData(
        remote_path=f'/users/{user_name}/resources/flexpart/IGBP_int1.dat',
